...
export default function SingleProductForm({ onBack }) {
    const router = useRouter();
    const API = process.env.NEXT_PUBLIC_API_URL;

    const [productData, setProductData] = useState({
        name: "", name_eng: "", cost: "", detail: "", stock_qty: "",
        brand: "", category_l1: "", category_l2: "", key_features: "",
        key_ingredients: "", suitable_for_concern: "", size_volume: "",
        usage_instructions: "", notes: ""
    });
    ...
    const processFiles = (files) => {
        ...
        if (validFiles.length !== files.length) alert("Only .pdf and .txt files are allowed.");
    ...
    const handleExtract = async () => {
        if (uploadedFiles.length === 0) { setExtractError("Please upload at least one file first."); return; }
        setIsExtracting(true); setExtractError("");
        try {
            const formData = new FormData();
            formData.append("file", uploadedFiles[0]);
            const res = await fetch(`${API}/admin/extract_file`, { method: "POST", body: formData });
            if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err.detail || `Error ${res.status}`); }
            applyExtractedData(await res.json());
        } catch (err) {
    ...
    const handleGoogleExtract = async () => {
        if (!googleUrl.trim()) { setExtractError("Please enter a Google Docs or Sheets URL."); return; }
        setIsExtracting(true); setExtractError("");
        try {
            const res = await fetch(`${API}/admin/google_extract_file?url=${encodeURIComponent(googleUrl)}`, { method: "POST" });
            if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err.detail || `Error ${res.status}`); }
            applyExtractedData(await res.json());
        } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch(`${API}/admin/add_product`, {
                ...
                body: JSON.stringify({ ...productData, cost: parseFloat(productData.cost) || 0 }),
            });
            if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err.detail || `Error ${res.status}`); }
        ...
        <div className={styles.layout}>
            ...
            <main className={styles.main}>
                <div className={styles.header}>
                    <div>
                        <button className={styles.backBtn} onClick={onBack}>← เลือกวิธีเพิ่มสินค้า</button>
                ...
                <div className={styles.contentCard}>
                    <form className={styles.formContainer} onSubmit={handleSubmit}>
                        <div className={styles.splitLayout}>
                            <div className={styles.formSection}>
                                ...
                                <div className={styles.inputGroup}>
                                    ...
                                    <input type="text" id="name" name="name" value={productData.name} onChange={handleInputChange} className={styles.input} placeholder="Product name" required />
                                </div>
                                <div className={styles.inputGroup}>
                                    <label htmlFor="name_eng" className={styles.label}>Product Name (Eng)</label>
                                    <input type="text" id="name_eng" name="name_eng" value={productData.name_eng} onChange={handleInputChange} className={styles.input} placeholder="Product name (eng)" required />
                                </div>
                                <div className={styles.row}>
                                    <div className={styles.inputGroup}>
                                        <label htmlFor="cost" className={styles.label}>Price (฿)</label>
                                        <input type="number" id="cost" name="cost" value={productData.cost} onChange={handleInputChange} className={styles.input} placeholder="0.00" min="0" step="0.01" required />
                                    </div>
                                    <div className={styles.inputGroup}>
                                        <label htmlFor="stock_qty" className={styles.label}>Stock Quantity</label>
                                        <input type="number" id="stock_qty" name="stock_qty" value={productData.stock_qty} onChange={handleInputChange} className={styles.input} placeholder="0" min="0" required />
                                    </div>
                                </div>
                                <div className={styles.row}>
                                    <div className={styles.inputGroup}>
                                ...
                                <div className={styles.inputGroup}>
                                    ...
                                    <textarea id="detail" name="detail" value={productData.detail} onChange={handleInputChange} className={styles.textarea} placeholder="Product detail..." rows="3" />
                                </div>
                                <div className={styles.inputGroup}>
                            ...
                            <div className={styles.formSection}>
                                ...
                                <div className={styles.modeTabs}>
                                    <button type="button" className={`${styles.modeTab} ${uploadMode === "file" ? styles.modeTabActive : ""}`} onClick={() => { setUploadMode("file"); setExtractError(""); }}>Upload File</button>
                                    <button type="button" className={`${styles.modeTab} ${uploadMode === "google" ? styles.modeTabActive : ""}`} onClick={() => { setUploadMode("google"); setExtractError(""); }}>Google Link</button>
                                </div>

                                {uploadMode === "file" && (
                                    <>
                                        <p className={styles.helperText}>Upload product specifications in .pdf or .txt formats.</p>
                                        <div className={`${styles.dropZone} ${isDragging ? styles.dragging : ""}`} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onClick={() => document.getElementById("fileUpload").click()}>
                                            <input type="file" id="fileUpload" multiple accept=".pdf,.txt" className={styles.hiddenInput} onChange={handleFileInput} />
                                            <div className={styles.dropZoneContent}>
                                                <svg className={styles.uploadIcon} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>
                                        ...
                                        {uploadedFiles.length > 0 && (
                                            <button type="button" className={styles.extractBtn} onClick={handleExtract} disabled={isExtracting}>
                                                {isExtracting ? <><span className={styles.spinner}></span> Extracting...</> : <><svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" /></svg> Extract with AI</>}
                                            </button>
                                        )}
                                    </>
                                )}
                                ...
                                {uploadMode === "google" && (
                                    <>
                                        <p className={styles.helperText}>Paste a public Google Docs or Google Sheets link.</p>
                                        <div className={styles.googleInputRow}>
                                            <input type="url" className={styles.input} placeholder="https://docs.google.com/document/d/..." value={googleUrl} onChange={(e) => setGoogleUrl(e.target.value)} />
                                        </div>
                                        <button type="button" className={styles.extractBtn} onClick={handleGoogleExtract} disabled={isExtracting || !googleUrl.trim()}>
                                            {isExtracting ? <><span className={styles.spinner}></span> Extracting...</> : <><svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" /></svg> Extract with AI</>}
                                        </button>
                                    </>
                                )}
                        ...
                        <div className={styles.actions}>
                            <button type="button" className={styles.cancelBtn} onClick={onBack} disabled={isSubmitting}>Cancel</button>
                            <button type="submit" className={styles.submitBtn} disabled={isSubmitting}>
                                {isSubmitting ? <><span className={styles.spinner}></span> Adding...</> : "Add Product"}
                            </button>
                        </div>
                    </form>
                </div>

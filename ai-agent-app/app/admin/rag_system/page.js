...
export default function rag_system() {
    ...
    async function onCreate(e) {
        ...
        try {
            ...
            if (!r.ok) {
                throw new Error(j?.detail || j?.message || "สร้างไม่สำเร็จ");
            }
    ...
    async function handleUpload(e) {
        ...
        if (!uploadFile) {
            setUploadErr("กรุณาเลือกไฟล์ก่อน");
            return;
        }
        if (!targetCollection) {
            setUploadErr("กรุณาเลือก collection");
            return;
        }
        ...
        try {
            ...
            fd.append("collection_name", targetCollection);
            ...
            const j = await r.json().catch(() => ({}));
            if (!r.ok || j.error) {
                throw new Error(j.error || j.detail || "อัปโหลดไม่สำเร็จ");
            }
            setUploadMsg(`อัปโหลดสำเร็จ • upload_id: ${j.upload_id} • chunks: ${j.chunks}`);
            setUploadFile(null);
        } catch (err) {
    ...
    async function fetchUploadHistory(col = "") {
    ...
    async function handleDeleteUpload(uploadId) {
        ...
        try {
            ...
            // ลบสำเร็จ: โหลดรายการใหม่ หรือจะลบออกจาก state ตรง ๆ ก็ได้
                                ...
                                <tbody>
                                    {collections.map((c) => (
                                        <tr key={c.id} style={{ borderBottom: "1px solid #e5e7eb" }}>
                                            ...
                                            <td style={{ padding: "8px 10px" }}>
                                                <button style={{ padding: "4px 8px", border: "none", borderRadius: "4px", background: "#ef4444", color: "white", cursor: "pointer", fontSize: "13px", }} onClick={() => handleDelete(c.name)}>
                                                    Delete</button>
                                            </td>
                                        </tr>
                                    ))}
